    ...
    const animate = () => {
        ...
        for (let i = 0; i < bubbles.length; i++) {
            ...
            if (fragmentation > 0 && Math.random() < fragmentation * 0.005) {
                for(let k=0; k<32; k++) {
                    spawnParticle(b.x, b.y, b.z, b.color);
                }
            ...
            if (Math.abs(magneto - 0.5) > 0.02) { 
                const magIntensity = (magneto - 0.5) * 2; 
                for (let j = 0; j < bubbles.length; j++) {
                    if (i === j) continue;
                    const other = bubbles[j];
                    const dx = other.x - b.x; const dy = other.y - b.y; const dz = other.z - b.z;
                    const distSq = dx*dx + dy*dy + dz*dz;
